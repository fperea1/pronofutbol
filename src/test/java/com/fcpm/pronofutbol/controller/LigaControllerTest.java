...
public class LigaControllerTest {
	...
	void testFindForSelect() throws Exception {
		...
		mockMvc
		    .perform(get(Constantes.LIGAS + Constantes.FIND_FOR_SELECT)
		    .header("authorization", "Bearer " + token))
		    .andDo(print())
		    .andExpect(status().isOk())
		    .andExpect(jsonPath("$.length()").isNotEmpty())
		    .andExpect(jsonPath("$.[0].nombre").value("Bundesliga"));
	}
	...
	void testSave() throws Exception {
		...
		LigaDTO u = getDTO("nombre test");
		
		String requestJson = getJson(u);
		
		mockMvc
		    .perform(post(Constantes.LIGAS + Constantes.SAVE)
		    .contentType(APPLICATION_JSON_UTF8)
		    .content(requestJson)
		    .header("authorization", "Bearer " + token))
		    .andExpect(status().isOk());	
	...
	void testFindByFilter() throws Exception {
		...
		mockMvc
		    .perform(get(Constantes.LIGAS + Constantes.FIND_BY_FILTER)
		    .param("filtro", filtro)
		    .header("authorization", "Bearer " + token))
		    .andDo(print())
		    .andExpect(status().isOk())
	...
	void testGetById() throws Exception {
		...
		ResultActions response = mockMvc
			    .perform(get(Constantes.LIGAS + Constantes.GET_BY_ID)
			    .param("id", "1")
			    .header("authorization", "Bearer " + token));
				
		LigaDTO q = getObjectFromJson(response.andReturn().getResponse().getContentAsString());
		assertNotNull(q);
	}
	...
	private LigaDTO getDTO(String nombre) {
	...
	private String getJson(LigaDTO u) throws JsonProcessingException {
		ObjectMapper mapper = new ObjectMapper();
	    mapper.configure(SerializationFeature.WRAP_ROOT_VALUE, false);
	    ObjectWriter ow = mapper.writer().withDefaultPrettyPrinter();
	...
	private LigaDTO getObjectFromJson(String s) throws JsonMappingException, JsonProcessingException {
	    ObjectReader jsonObjectReader = new ObjectMapper().readerFor(LigaDTO.class);
	    LigaDTO q = jsonObjectReader.readValue(s);
	    return q;
	}
