...
public class JornadaControllerTest {
	...
	void testSave() throws Exception {
		...
		JornadaDTO u = getDTO("nombre test");
		
		String requestJson = getJson(u);
		
		mockMvc
		    .perform(post(Constantes.JORNADAS + Constantes.SAVE)
		    .contentType(APPLICATION_JSON_UTF8)
		    .content(requestJson)
	...
	void testFindForSelect() throws Exception {
		...
		mockMvc
		    .perform(get(Constantes.JORNADAS + Constantes.FIND_FOR_SELECT)
		    .header("authorization", "Bearer " + token))
		    .andDo(print())
		    .andExpect(status().isOk())
	...
	void testFindByFilter() throws Exception {
		...
		mockMvc
		    .perform(get(Constantes.JORNADAS + Constantes.FIND_BY_FILTER)
		    .param("filtro", filtro)
		    .header("authorization", "Bearer " + token))
		    .andDo(print())
		    .andExpect(status().isOk())
	...
	void testGetById() throws Exception {
		...
		ResultActions response = mockMvc
			    .perform(get(Constantes.JORNADAS + Constantes.GET_BY_ID)
			    .param("id", "1")
			    .header("authorization", "Bearer " + token));
				
		JornadaDTO q = getObjectFromJson(response.andReturn().getResponse().getContentAsString());
		assertNotNull(q);
	}
	...
	private JornadaDTO getDTO(String nombre) {
		JornadaDTO dto = new JornadaDTO();
		dto.setNombre(nombre);
		return dto;
	}

	private String getJson(JornadaDTO u) throws JsonProcessingException {
		ObjectMapper mapper = new ObjectMapper();
	    mapper.configure(SerializationFeature.WRAP_ROOT_VALUE, false);
	    ObjectWriter ow = mapper.writer().withDefaultPrettyPrinter();
	...
	private JornadaDTO getObjectFromJson(String s) throws JsonMappingException, JsonProcessingException {
	    ObjectReader jsonObjectReader = new ObjectMapper().readerFor(JornadaDTO.class);
	    JornadaDTO q = jsonObjectReader.readValue(s);
	    return q;
	}
